...
public class Listener<TK, TV> where TV : class
{
    ...
    private HashSet<TV> autoRemoveMapper = new HashSet<TV>();
    private List<KeyValuePair<TK, TV>> autoRemoveList = new List<KeyValuePair<TK, TV>>(10);
    ...
    public bool Dispatch(TK tk, Action<TV> dispatchAction)
    {
        ...
        if (listeners.TryGetValue(tk, out List<TV> list))
        {
            ...
            {
                var val = list[i];
                dispatchAction?.Invoke(val);
                hasDispatch = true;

                if (autoRemoveMapper.Contains(val))
                    autoRemoveList.Add(new KeyValuePair<TK, TV>(tk, list[i]));
            }
        }

        foreach (var e in autoRemoveList)
        {
